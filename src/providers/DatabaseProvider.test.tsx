...
describe("DatabaseProvider", () => {
	...
	it("creates tracked database and refreshData behavior", async () => {
		...
		const incrementVersion = stateSetters.setDataVersion.mock
			.calls[0]?.[0] as (currentVersion: number) => number;
		...
		const incrementPending = stateSetters.setPendingOperations.mock
			.calls[0]?.[0] as (current: number) => number;
		expect(incrementPending(3)).toBe(4);
